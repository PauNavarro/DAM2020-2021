...
public class Pong extends JPanel implements KeyListener {
	...
	// Variables para la posicion del rectangulo superior
	...
	// Metodo para el movimiento del rectangulo superior

	public void movimientoRectArriba() {

		// Hacer que pare de moverse en caso de que toque uno de los margenes

		if (xRectAr + xaRectAr >= getWidth() - 799) {
			xaRectAr = 0;
		}
		if (xRectAr + xaRectAr >= getWidth() - 150) {
			xaRectAr = 0;
		}

		// Detección de pulsaciones y controles.

		if (pulsacion == KeyEvent.VK_LEFT) {

			if (xRectAr + xaRectAr < 0) {
				xaRectAr = 0;
			} else {
				xaRectAr = -2;
			}
		...
		if (pulsacion == KeyEvent.VK_RIGHT) {
			if (xRectAr + xaRectAr >= getWidth() - 150) {
				xaRectAr = 0;
			} else {
				xaRectAr = 2;
		...
		xRectAr = xRectAr + xaRectAr;

		// Inizializar el vector utilizado para comprobar colision

		int a = 1;

		for (int i = 0; i < posicionRectAr.length; i++) {
			posicionRectAr[i] = xRectAr + a;
			a++;
		}
	...
	private int[] movimientoPelota(int resultado[]) {
		...
		// Comprobar si hay colision con el rectangulo de arriba.

		if (y + ya < 200)

			for (int i = 0; i < posicionRectAr.length; i++) {
				if (x == posicionRectAr[i] && y == 30) {
					ya = 1;
					puntuacion++;
	...
	public void paint(Graphics g) {
		...
		g.fillRect(xRectAr, 30, 150, 30); // Rectangulo de arriba
	...
	public static void main(String[] args) throws InterruptedException {
		...
		do {
			...
			pong.movimientoRectArriba(); // Movimiento rectangulo de arriba
